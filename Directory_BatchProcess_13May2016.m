...
listing = dir(fullfile(sourceDir,'*.czi'));
numSourceFiles = numel(listing);
sourcePaths = cell(1,numSourceFiles);
filenames = cell(1,numSourceFiles);
...
if parallel_switch
    ...
    for ff = 1:numSourceFiles
        ...
        thisPath = fullfile(sourceDir,listing(ff).name);
...
parfor ff = 1:numSourceFiles
    ...
    filenames{ff} = listing(ff).name;
    
    if ~parallel_switch
        
        fprintf('Accessing file %d of %d files (%s):\n',...
            ff,numSourceFiles,filenames{ff})
...
saveDir = [sourceDir,filesep,'AnalysisResults',filesep];
...
for ff = 1:numSourceFiles
    ...
    for ee = 1:numEmbryos
        ...
        saveFileName = [saveDir,filenames{ff},sprintf('_%d_%d.mat',ff,ee)];
